...
class IngredientListTest extends TestCase
{
     ...
     */
    public function testLimit(int $ingredientsCreated, string $limit)
    {
        $user = User::factory()->create();
        Ingredient::factory($ingredientsCreated)->create();

        $this->actingAs($user)
            ->getJson(sprintf('api/ingredients?limit=%s', $limit))
            ->assertOk()
     ...
     */
    public function testInvalidLimit(int $ingredientsCreated, string $limit)
    {
        $user = User::factory()->create();
        Ingredient::factory($ingredientsCreated)->create();
     ...
     *  */
    public function testPagination(int $ingredientsCreated, string $limit, string $page, int $assertCount)
    {
        $user = User::factory()->create();
        Ingredient::factory($ingredientsCreated)->create();

        $this->actingAs($user)
            ->getJson(sprintf('api/ingredients?limit=%s&page=%s', $limit, $page))
            ->assertOk()
    ...
    {
        $user = User::factory()->create();
        Ingredient::factory(7)->create();

        $this->actingAs($user)
            ->getJson('api/ingredients?limit=5')
            ->assertOk()
    ...
    public function testPageNotfoundOffset()
    {
        ...
        $this->actingAs($user)
            ->getJson('api/ingredients?limit=5&page=50')
            ->assertOk()
    ...
    public function testInvalidPagination(string $page)
    {
        ...
        $this->actingAs($user)
            ->getJson(sprintf('api/ingredients?limit=12&page=%s', $page))
            ->assertUnprocessable();
    }
