...
class ClientListTest extends TestCase
{
     ...
     */
    public function testLimit(int $clientsCreated, string $limit)
    {
        $user = User:: factory()->create();
              Client:: factory($clientsCreated)->create();

        $this->actingAs($user)
            ->getJson(sprintf('api/clients?limit=%s', $limit))
            ->assertOk()
     ...
     */
    public function testInvalidLimit(int $clientsCreated, string $limit)
    {
        $user = User:: factory()->create();
              Client:: factory($clientsCreated)->create();
     ...
     *  */
    public function testPagination(int $clientsCreated, string $limit, string $page, int $assertCount)
    {
        $user = User:: factory()->create();
              Client:: factory($clientsCreated)->create();

        $this->actingAs($user)
            ->getJson(sprintf('api/clients?limit=%s&page=%s', $limit, $page))
            ->assertOk()
    ...
    public function testPageNotfoundOffset()
    {
        ...
        $this->actingAs($user)
            ->getJson('api/clients?limit=5&page=50')
            ->assertOk()
    ...
    public function testInvalidPagination(string $page)
    {
        ...
        $this->actingAs($user)
            ->getJson(sprintf('api/clients?limit=12&page=%s', $page))
            ->assertUnprocessable();
    }
